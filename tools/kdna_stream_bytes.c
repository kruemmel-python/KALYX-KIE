...
static void usage(void) {
    fprintf(stderr,
        "kdna_stream_bytes — arbitrary bytes -> canonical uint64 KSTREAM\n"
        "usage:\n"
        "  kdna_stream_bytes --in file.bin --out file.u64 --summary file.kstream [--mode byte|window] [--window N] [--stride N] [--max-symbols 0]\n"
        "  kdna_stream_bytes --a file.kstream\n");
}

int main(int argc, char **argv) {
    const char *in_path = NULL, *out_path = NULL, *summary_path = NULL, *inspect_path = NULL;
    const char *mode = "byte";
    uint32_t window = 8u, stride = 1u;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (strcmp(argv[i], "--window") == 0 && i + 1 < argc) { if (!parse_u32(argv[++i], &window) || window == 0u || window > 65536u) { usage(); return 2; } }
        else if (strcmp(argv[i], "--stride") == 0 && i + 1 < argc) { if (!parse_u32(argv[++i], &stride) || stride == 0u) { usage(); return 2; } }
    ...
    uint64_t input_bytes = 0, input_records = 0, symbols = 0, skipped = 0, invalid = 0;
    ...
    if (strcmp(mode, "byte") == 0) {
        ...
        unsigned char *buf = (unsigned char *)malloc((size_t)window);
        if (!buf) { fclose(in); fclose(out); fprintf(stderr, "out of memory\n"); return 2; }
        size_t filled = 0;
        uint64_t pos = 0;
        int c;
        while ((c = fgetc(in)) != EOF) {
            ...
            if (filled < window) {
                ...
                memmove(buf, buf + 1, (size_t)window - 1u);
                buf[window - 1u] = b;
            }
            if (filled == window) {
                if ((pos % (uint64_t)stride) == 0u) {
                    uint64_t v = fnv1a_update(FNV64_OFFSET, buf, (size_t)window);
                    if (!write_u64(out, v, &symbol_hash)) { free(buf); fclose(in); fclose(out); return 2; }
                    symbols++;
                    input_records++;
                    if (max_symbols != 0u && symbols >= max_symbols) break;
                } else {
        ...
        fclose(in); fclose(out); usage(); return 2;
    }
    ...
    h.version = KDNA_KSTREAM_VERSION;
    h.header_bytes = KDNA_KSTREAM_HEADER_BYTES;
    h.kind = KDNA_KSTREAM_KIND_BYTES;
    ...
    printf("kdna_stream_bytes: in=%s out=%s summary=%s mode=%s symbols=%" PRIu64 " input_bytes=%" PRIu64 " hash=0x%016" PRIx64 "\n",
           in_path, out_path, summary_path, mode, symbols, input_bytes, symbol_hash);
    return 0;
}
